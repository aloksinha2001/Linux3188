...
/* Control Packet Auto Send With VYSNC control */
#define CONTROL_PACKET_AUTO_SEND	0x104
	#define m_CONTROL_PACKET_AUTO_GC	(1 << 7)
	#define m_CONTROL_PACKET_AUTO_MPEG	(1 << 6)
	#define m_CONTROL_PACKET_AUTO_SPD	(1 << 5)
	#define m_CONTROL_PACKET_AUTO_VS	(1 << 4)
	#define m_CONTROL_PACKET_AUTO_GMT	(1 << 3)
	#define m_CONTROL_PACKET_AUTO_ISRC	(1 << 2)
	#define m_CONTROL_PACKET_AUTO_ACP	(1 << 1)
	#define m_CONTROL_PACKET_AUTO_GP	(1 << 0)
...
#define HDMIRdReg(addr)						__raw_readl(rk30_hdmi->regbase + addr)
#define HDMIWrReg(addr, val)        		__raw_writel((val), rk30_hdmi->regbase + addr);
#define HDMIMskReg(temp, addr, msk, val)	\
	temp = __raw_readl(rk30_hdmi->regbase + addr) & (0xFF - (msk)) ; \
	__raw_writel(temp | ( (val) & (msk) ),  rk30_hdmi->regbase + addr); 
...
extern int rk30_hdmi_disable(struct hdmi *hdmi);
extern int rk30_hdmi_detect_hotplug(struct hdmi *hdmi);
extern int rk30_hdmi_read_edid(struct hdmi *hdmi, int block, unsigned char *buff);
extern int rk30_hdmi_removed(struct hdmi *hdmi);
extern int rk30_hdmi_config_video(struct hdmi *hdmi, struct hdmi_video *vpara);
extern int rk30_hdmi_config_audio(struct hdmi *hdmi, struct hdmi_audio *audio);
extern int rk30_hdmi_control_output(struct hdmi *hdmi, int enable);
extern int rk30_hdmi_config_vsi(struct hdmi *hdmi, unsigned char vic_3d, unsigned char format);
#endif
