...
static void rk30_hdcp_write_mem(struct rk30_hdmi *rk30_hdmi, int addr_8, char value)
{
	int temp;
...
int rk30_hdcp_load_key2mem(struct hdcp *hdcp, struct hdcp_keys *key)
{
	...
	for(i = 0; i < 7; i++)
		rk30_hdcp_write_mem(rk30_hdmi, HDCP_RAM_KEY_KSV1 + i, key->KSV[i]);
	for(i = 0; i < 7; i++)
		rk30_hdcp_write_mem(rk30_hdmi, HDCP_RAM_KEY_KSV2 + i, key->KSV[i]);
	for(i = 0; i < HDCP_PRIVATE_KEY_SIZE; i++)
		rk30_hdcp_write_mem(rk30_hdmi, HDCP_RAM_KEY_PRIVATE + i, key->DeviceKey[i]);
	for(i = 0; i < HDCP_KEY_SHA_SIZE; i++)
		rk30_hdcp_write_mem(rk30_hdmi, HDCP_RAM_KEY_PRIVATE + HDCP_PRIVATE_KEY_SIZE + i, key->sha1[i]);
...
int rk30_hdcp_start_authentication(struct hdcp *hdcp)
{
	...
	// Config DDC Clock
	temp = (rk30_hdmi->tmdsclk/HDCP_DDC_CLK)/4;
...
int rk30_hdcp_check_bksv(struct hdcp *hdcp)
{
	int i, temp;
	char bksv[5];
	char *invalidkey;
