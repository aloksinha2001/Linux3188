...
static struct delayed_work *hdcp_submit_work(int event, int delay)
{
	...
	HDCPDBG("%s event %04x delay %d", __FUNCTION__, event, delay);
...
static void hdcp_wq_authentication_failure(void)
{
	...
	rk2928_hdcp_disable(hdcp);
	rk2928_hdmi_control_output(hdcp->hdmi, HDMI_VIDEO_MUTE | HDMI_AUDIO_MUTE);
...
static void hdcp_wq_start_authentication(void)
{
	...
	HDCPDBG("HDCP: authentication start");

	status = rk2928_hdcp_start_authentication(hdcp);

	if (status != HDCP_OK) {
		HDCPDBG("HDCP: authentication failed");
...
static void hdcp_wq_check_bksv(void)
{
	...
	HDCPDBG("Check BKSV start");
	
	status = rk2928_hdcp_check_bksv(hdcp);
	...
	if (status != HDCP_OK) {
		...
		HDCPDBG("HDCP: Check BKSV successful");
...
{
	rk2928_hdmi_control_output(hdcp->hdmi, HDMI_AV_UNMUTE);
...
static void hdcp_work_queue(struct work_struct *work)
{
	...
	HDCPDBG("hdcp_work_queue() - START - %u hdmi=%d hdcp=%d evt= %x %d",
...
static void hdcp_irq_cb(int status)
{
	...
	rk2928_hdcp_interrupt(hdcp, &interrupt1, &interrupt2);
	HDCPDBG("%s 0x%02x 0x%02x", __FUNCTION__, interrupt1, interrupt2);
...
static void hdcp_load_keys_cb(const struct firmware *fw, void *context)
{
	...
	if(fw->size > HDCP_KEY_SIZE) {
		HDCPDBG("%s invalid key size %d", __FUNCTION__, fw->size - HDCP_KEY_SIZE);
...
static int __init rk2928_hdcp_init(void)
{
	...
	HDCPDBG("[%s] %u", __FUNCTION__, jiffies_to_msecs(jiffies));
	...
	hdcp->hdmi = rk2928_hdmi_register_hdcp_callbacks(hdcp_start_frame_cb,
										hdcp_irq_cb,
										hdcp_power_on_cb,
										hdcp_power_off_cb);
										
	HDCPDBG("%s success %u", __FUNCTION__, jiffies_to_msecs(jiffies));
	return 0;
