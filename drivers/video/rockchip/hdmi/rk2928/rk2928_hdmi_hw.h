...
#define HDMIRdReg(addr)						readl_relaxed(rk2928_hdmi->regbase + (addr) * 0x04)
#define HDMIWrReg(addr, val)        		writel_relaxed((val), rk2928_hdmi->regbase + (addr) * 0x04)
#define HDMIMskReg(temp, addr, msk, val)	\
	temp = readl_relaxed(rk2928_hdmi->regbase + (addr) * 0x04) & (0xFF - (msk)) ; \
	writel_relaxed(temp | ( (val) & (msk) ),  rk2928_hdmi->regbase + (addr) * 0x04); 
	
extern void rk2928_hdmi_reset(struct rk2928_hdmi *rk2928_hdmi);
extern int rk2928_hdmi_enable(struct hdmi *hdmi);
extern int rk2928_hdmi_disable(struct hdmi *hdmi);
extern int rk2928_hdmi_detect_hotplug(struct hdmi *hdmi);
extern int rk2928_hdmi_read_edid(struct hdmi *hdmi, int block, unsigned char *buff);
extern int rk2928_hdmi_removed(struct hdmi *hdmi);
extern int rk2928_hdmi_config_video(struct hdmi *hdmi, struct hdmi_video *vpara);
extern int rk2928_hdmi_config_audio(struct hdmi *hdmi, struct hdmi_audio *audio);
extern int rk2928_hdmi_control_output(struct hdmi *hdmi, int enable);
#endif
