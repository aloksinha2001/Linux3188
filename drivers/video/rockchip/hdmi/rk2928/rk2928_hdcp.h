...
#ifdef HDCP_DEBUG
#define HDCPDBG(format, ...) \
		printk(KERN_INFO "HDCP: " format "\n", ## __VA_ARGS__)
#else
#define HDCPDBG(format, ...)
#endif

extern void rk2928_hdcp_disable(struct hdcp *hdcp);
extern int	rk2928_hdcp_start_authentication(struct hdcp *hdcp);
extern int	rk2928_hdcp_check_bksv(struct hdcp *hdcp);
extern int	rk2928_hdcp_load_key2mem(struct hdcp *hdcp, struct hdcp_keys *key);
extern void rk2928_hdcp_interrupt(struct hdcp *hdcp, char *status1, char *status2);
