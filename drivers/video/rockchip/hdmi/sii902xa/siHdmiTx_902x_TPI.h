...
typedef struct
{
    ...
    byte opcode;
    byte args[ SII_MAX_CMD_SIZE ];
    byte argCount;
   	byte nextFrameArgCount;
} SI_CpiData_t;
...
enum
{
    ...
    SI_CECTASK_SENDMSG,
    SI_CECTASK_StartActive	//2011.08.03 Start Active Source
};

typedef struct
{
    byte deviceType;     // 0 - Device is a TV.
                            // 1 - Device is a Recording device
                            // 2 - Device is a reserved device
                            // 3 - Device is a Tuner
                            // 4 - Device is a Playback device
                            // 5 - Device is an Audio System
    byte  cecLA;         // CEC Logical address of the device.
    word cecPA;         // CEC Physical address of the device.
} CEC_DEVICE;

extern byte g_cecAddress;       // Initiator
extern word g_cecPhysical;      // For TV, the physical address is 0.0.0.0
...
bool SI_CecEnumerate( byte nexttask1,byte nexttask2 );//2011.08.03 
...
void SI_CecSendImageViewOn(void);//2011.08.03 
void SI_CecSendActiveSource(void);//2011.08.03 Send Image view on, Active Source

byte CECTaskStartActive( SI_CpiStatus_t *pCecStatus );//2011.08.03 logical address
void SI_StartActiveSource ( byte nexttask1, byte nexttask2 );//2011.08.03 logical address
#endif//#ifdef DEV_SUPPORT_CEC
#endif
