...
extern void SII902x_Reset(void);
...
{
	TPI_TRACE_PRINT((">>EnableInterrupts() %02x\n", Interrupt_Pattern));
	ReadSetWriteTPI(TPI_INTERRUPT_ENABLE_REG, Interrupt_Pattern);
	TPI_TRACE_PRINT((">>EnableInterrupts() value 0x%02x\n", ReadByteTPI(TPI_INTERRUPT_ENABLE_REG)));
	return TRUE;
}
...
static byte g_CommData [EDID_BLOCK_SIZE];
		...
		if (HDCP_Rev == 0x00)
		{
			...
			if (HDCP_Rev == 0x02)
			{
				...
				if (HDCP_Rev == 0x02)
				{
					...
					{
						HDCP_Supported = FALSE;
						TPI_TRACE_PRINT((">>SiI9022A found, NO HDCP supported\n"));
					}
				}
			}
		}
...
//					HDCP_On();
		...
		{
			RegImage = ReadByteTPI(TPI_HDCP_QUERY_DATA_REG);
//			TPI_DEBUG_PRINT(("HDCP_CHANGE_EVENT TPI_HDCP_QUERY_DATA_REG %02x\n", RegImage));
...
//------------------------------------------------------------------------------
static const byte AspectRatioTable[] =
{
...
byte SetAVI_InfoFrames (void)
{
	...
	B_Data[1] |= 0x10;  // A0 = 1; Active format identification data is present in the AVI InfoFrame. // S1:S0 = 01; Overscanned (television).
						// S1:S0 = 00; No Data
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	if ((g_hdcp.HDCP_TxSupports == TRUE) && (g_hdcp.HDCPAuthenticated == VMD_HDCP_AUTHENTICATED))
	{
		if (g_hdcp.HDCP_AksvValid == TRUE)
		{
			...
			g_sys.tmdsPoweredUp = TRUE;
			#ifdef DEV_SUPPORT_CEC
			EnableInterrupts(HOT_PLUG_EVENT | RX_SENSE_EVENT | AUDIO_ERROR_EVENT | SECURITY_CHANGE_EVENT | HDCP_CHANGE_EVENT|0x08);
			#else
...
#endif
	{
		...
		g_sys.tmdsPoweredUp = TRUE;
		#ifdef DEV_SUPPORT_CEC
		EnableInterrupts(HOT_PLUG_EVENT | RX_SENSE_EVENT | AUDIO_ERROR_EVENT|0x08);
		#else
...
//-------------------------------------------------------------------------------
//	Following function added for hdmi_itv driver.
//-------------------------------------------------------------------------------
int siHsiHdmiTx_Detect(void)
{
	TPI_TRACE_PRINT(("\n>>siHsiHdmiTx_Detect()\n"));
	TPI_TRACE_PRINT(("\n%s\n", TPI_FW_VERSION));
...
int siHsiHdmiTx_ReadEdid(byte Block, byte* buf)
{
	byte SysCtrlReg;
	byte Offset, Segment;
    ...
    if (Block < 2)
    {
        ReadBlockEDID(Offset, EDID_BLOCK_SIZE, buf);    // read first Segment of EDID ROM
    }
    else
    {
        ReadSegmentBlockEDID(Segment, Offset, EDID_BLOCK_SIZE, buf);     // read next Segment of EDID ROM
    }
...
int siHsiHdmiTx_VideoConfig(byte isHDMI)
{
	DisableTMDS();
	#ifdef DEV_SUPPORT_HDCP
	if(g_hdcp.HDCP_Started == TRUE)
...
int sii902x_sys_init(void)
{
	return (siHdmiTx_TPI_Init() == TRUE)?HDMI_ERROR_SUCESS:HDMI_ERROR_FALSE;
}

void sii902x_irq_poll(struct hdmi *hdmi)
{
	byte InterruptStatus = 0;
		
	if (g_sys.txPowerState == TX_POWER_STATE_D0)
	{
		InterruptStatus = ReadByteTPI(TPI_INTERRUPT_STATUS_REG);
//		printk("InterruptStatus %02x\n", InterruptStatus);
		// Poll Hot Plug Status
		if (InterruptStatus & HOT_PLUG_EVENT)
		{
			TPI_DEBUG_PRINT (("HPD  -> "));
			ReadSetWriteTPI(TPI_INTERRUPT_ENABLE_REG, HOT_PLUG_EVENT);  // Enable HPD interrupt bit
...
int sii902x_detect_hotplug(struct hdmi *hdmi)
{
	return (g_sys.hdmiCableConnected == TRUE)?HDMI_HPD_ACTIVED:HDMI_HPD_REMOVED;
}

int sii902x_insert(struct hdmi *hdmi)
{
	TPI_DEBUG_PRINT (("Cable Connected\n"));
	#ifdef DEV_SUPPORT_HDCP
	if ((g_hdcp.HDCP_TxSupports == TRUE) 
...
int sii902x_remove(struct hdmi *hdmi)
{
	TPI_DEBUG_PRINT (("HDMI Disconnected\n"));
	OnDownstreamRxPoweredDown();
	return HDMI_ERROR_SUCESS;
...
int sii902x_read_edid(struct hdmi *hdmi, int block, unsigned char *buff)
{
	return (siHsiHdmiTx_ReadEdid(block, buff) == EDID_OK)?HDMI_ERROR_SUCESS:HDMI_ERROR_FALSE;
}

int sii902x_config_video(struct hdmi *hdmi, struct hdmi_video *vpara)
{
	// ÅäÖÃÊÓÆµÊä³ö
	byte vmode;
	switch(vpara->vic)
	...
	siHdmiTx_VideoSel(vmode);
	g_edid.YCbCr_4_4_4 = hdmi->edid.ycbcr444;
	g_edid.YCbCr_4_2_2 = hdmi->edid.ycbcr422;
	siHsiHdmiTx_VideoConfig(vpara->sink_hdmi);
	if(vpara->sink_hdmi) {
		if(vpara->format_3d != HDMI_3D_NONE)
			sii902x_config_vsi(hdmi, vpara->format_3d, HDMI_VIDEO_FORMAT_3D);
		else if(vpara->vic & HDMI_VIDEO_EXT)
			sii902x_config_vsi(hdmi, vpara->vic & HDMI_VIC_MASK, HDMI_VIDEO_FORMAT_4Kx2K);
		else
			sii902x_config_vsi(hdmi, vpara->vic, HDMI_VIDEO_FORMAT_NORMAL);
	}
	return HDMI_ERROR_SUCESS;
}

int sii902x_config_audio(struct hdmi *hdmi, struct hdmi_audio *audio)
{
	byte MCLK_FS;
	...
	siHdmiTx.AudioMode		= AMODE_I2S;
	siHdmiTx.AudioChannels	= audio->channel - 1;
	
	switch(audio->rate)
	{
		case HDMI_AUDIO_FS_32000:
			siHdmiTx.AudioFs = AFS_32K;
			MCLK_FS = MCLK384FS;
			break;
		case HDMI_AUDIO_FS_44100:
			siHdmiTx.AudioFs = AFS_44K1;
			MCLK_FS = MCLK256FS;
			break;
		case HDMI_AUDIO_FS_48000:
			siHdmiTx.AudioFs = AFS_48K;
			MCLK_FS = MCLK256FS;
			break;
		case HDMI_AUDIO_FS_88200:
			siHdmiTx.AudioFs = AFS_88K2;
			MCLK_FS = MCLK128FS;
			break;
		case HDMI_AUDIO_FS_96000:
			siHdmiTx.AudioFs = AFS_96K;
			MCLK_FS = MCLK128FS;
			break;
		case HDMI_AUDIO_FS_176400:
			siHdmiTx.AudioFs = AFS_176K4;
			MCLK_FS = MCLK128FS;
			break;
		case HDMI_AUDIO_FS_192000:
			siHdmiTx.AudioFs = AFS_192K;
			MCLK_FS = MCLK128FS;
			break;
		default:
	...
	switch(audio->word_length)
	{
		...
		case HDMI_AUDIO_WORD_LENGTH_20bit:
			siHdmiTx.AudioWordLength = ALENGTH_20BITS;
			break;
	...
	siHdmiTx.AudioI2SFormat	= (MCLK_FS << 4) |SCK_SAMPLE_RISING_EDGE |0x00;
	
	siHdmiTx_AudioSet();
	return HDMI_ERROR_SUCESS;
}

int sii902x_control_output(struct hdmi *hdmi, int enable)
{
...
int sii902x_config_vsi(struct hdmi *hdmi, unsigned char vic_3d, unsigned char format)
{
	char info[SIZE_MPEG_INFOFRAME];
	int i;
	
	// Disable transmission of VSIF during re-configuration
	WriteByteTPI(MISC_INFO_FRAMES_CTRL, DISABLE_MPEG);
	
	memset(info, 0, SIZE_MPEG_INFOFRAME);
	// Header Bytes
	info[0] = 0x81;
	info[1] = 0x01;
	// PB1 - PB3 contain the 24bit IEEE Registration Identifier
...
//	for(i = 0; i < SIZE_MPEG_INFOFRAME; i++) {
//		printk("[%s] info[%d] is 0x%02x.\n", __FUNCTION__, i, info[i]);
//	}
	
	WriteByteTPI(MISC_INFO_FRAMES_CTRL, EN_AND_RPT_MPEG);         // Enable and Repeat MPEG/Vendor Specific InfoFrames
	
	WriteBlockTPI(MISC_INFO_FRAMES_TYPE, SIZE_MPEG_INFOFRAME, info);             // Write VSIF to MPEG registers and start transmission
	WriteByteTPI(0xDE, 0x00);		// Set last byte of MPEG InfoFrame for data to be sent to sink.
	return 0;
}

int sii902x_config_cec(struct hdmi *hdmi)
{
	#ifdef DEV_SUPPORT_CEC
	if (hdmi->edid.cecaddress != SI_CecGetDevicePA ())
	{
		// Yes!  So change the PA
		SI_CecSetDevicePA (hdmi->edid.cecaddress);
	}
	TPI_TRACE_PRINT(("physical address is 0x%x\n",(int)hdmi->edid.cecaddress));
	SI_CecEnumerate(SI_CECTASK_StartActive,0);
	#endif
	return HDMI_ERROR_SUCESS;
}