...
#ifdef SCAN_NO_RESULT_FIX
/* patch fo scan no result */
static int presist_cnt = 0;
//static int old_rxintcnt = 0;
static int old_rxdfrmmcast = 0;
static int old_rxmfrmmcast = 0;
static int old_rxmfrmocast = 0;
//static int cur_rxintcnt = 0;
static int cur_rxdfrmmcast = 0;
static int cur_rxmfrmmcast = 0;
static int cur_rxmfrmocast = 0;
static int ap_found = 0;
static int wl_iw_counters(struct net_device *dev);
...
wl_set_set_wapi_ie(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	if(sme->crypto.wpa_versions & NL80211_WAPI_VERSION_1)
	{
		err = wldev_iovar_setbuf_bsscfg(dev, "wapiie", sme->ie, sme->ie_len, ioctlbuf, sizeof(ioctlbuf), bssidx);
...
static s32 wl_inform_single_bss(struct wl_priv *wl, struct wl_bss_info *bi)
{
	...
	if (wl->p2p_supported && p2p_on(wl) && p2p_scan(wl)) { //penguin
		...
		if (wl_is_linkup(wl, e, ndev)) {
			...
			if (wl_is_ibssmode(wl, ndev)) {
				...
				if (!wl_get_drv_status(wl, DISCONNECTING)) {
					printf("wl_bss_connect_done succeeded status=(0x%x)\n",
						(int)wl->status);
			...
			if (wl_get_drv_status(wl, CONNECTED)) {
				scb_val_t scbval;
				printf("link down, call cfg80211_disconnected\n");
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (bss_list->count == 0) {
		...
		if (((presist_cnt == 0) && ap_found) || (presist_cnt == 1)) {
			...
			printk("### Hugh [%s], presist_cnt = %d\n", __FUNCTION__, presist_cnt);
			if (ap_found)
				err = -EINVAL;
			ap_found = 0;
		}
		...
		if (presist_cnt > 1) {
			if (((cur_rxmfrmocast - old_rxmfrmocast) || (cur_rxdfrmmcast - old_rxdfrmmcast)
				//|| (cur_rxmfrmmcast - old_rxmfrmmcast)) &&!(cur_rxintcnt -old_rxintcnt )) {
				|| (cur_rxmfrmmcast - old_rxmfrmmcast))) {
				printk("### Hugh [%s], System hang!!!\n",__FUNCTION__);
				//printk("### Hugh diff_rxintcnt[%d]\n",(cur_rxintcnt - old_rxintcnt));
				printk("### Hugh diff_rxdfrmmcas[%d]\n",(cur_rxdfrmmcast - old_rxdfrmmcast));
				printk("### Hugh diff_rxmfrmmcas[%d]\n",(cur_rxmfrmmcast - old_rxmfrmmcast));
				printk("### Hugh diff_rxmfrmocas[%d]\n",(cur_rxmfrmocast - old_rxmfrmocast));
				net_os_send_hang_message(ndev);
				presist_cnt = 0;
				cur_rxmfrmmcast = old_rxmfrmmcast = 0;
				cur_rxmfrmocast = old_rxmfrmocast = 0;
				cur_rxdfrmmcast = old_rxdfrmmcast = 0;
				//cur_rxintcnt = old_rxintcnt = 0;
				ap_found = 0;
				err = -EINVAL;
			} else {
				presist_cnt = 0;
		...
		old_rxdfrmmcast = cur_rxdfrmmcast;
		old_rxmfrmmcast = cur_rxmfrmmcast;
		old_rxmfrmocast = cur_rxmfrmocast;
		//old_rxintcnt = cur_rxintcnt;
	} else {
		presist_cnt = 0;
		cur_rxmfrmmcast = old_rxmfrmmcast = 0;
		cur_rxmfrmocast = old_rxmfrmocast = 0;
...
void wl_cfg80211_set_scan_abort(void)
{
	struct wl_priv *wl = wlcfg_drv_priv;
	struct net_device *ndev = NULL;

	if (wl&&wl->scan_request) {
		printf("abort scan when dongle down....\n");
...
{
	u8 b_mac[6] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};

	printf("deauth: %02X:%02X:%02X:%02X:%02x:%02X\n", buf[0], buf[1], buf[2],
		buf[3],buf[4],buf[5]);
	if (!memcmp(buf, b_mac, 6)) {
		return wl_iw_softap_deassoc_stations(net, NULL);
	}
...
int wl_cfg80211_set_power_mode(struct net_device *dev, char *command)
{
	...
	if ((strnicmp((char *)&powermode_val, "1",
		...
		WL_ERR(("%s: get corrupted POWERMODE=%s : enforce POWERMODE 0\n",
		__FUNCTION__, command));
	}
	...
	if ((strnicmp((char *)&powermode_val, "1", strlen("1")) == 0)&&
		...
		wldev_ioctl(dev, WLC_GET_PM, &pm, sizeof(int), false);
		wldev_ioctl(dev, WLC_SET_PM, &pm_local, sizeof(int), true);

		net_os_set_packet_filter(dev, 0);
	} else {
		set_power_mode_called = FALSE;
		wldev_ioctl(dev, WLC_SET_PM, &pm, sizeof(int), true);
...
#ifdef SCAN_NO_RESULT_FIX
#define	PRVAL(name)	if (cnt.name != cnt_old.name) pbuf += sprintf(pbuf, "%s %d ", #name, dtoh32(cnt.name)-dtoh32(cnt_old.name))
#define	PRNL()		pbuf += sprintf(pbuf, "\n")
#define PRD11VAL(name)  if (cnt.name != cnt_old.name) pbuf += sprintf(pbuf, "d11_%s %d ", #name, dtoh32(cnt.name)-dtoh32(cnt_old.name))

static wl_cnt_t cnt_old;
static int
wl_iw_counters(struct net_device *dev)
{
	wl_cnt_t cnt;
	uint i;
	char *pbuf;
	...
	if (!counter_buf) {
		printk("no enough mem!\n");
		return -ENOMEM;
	}

	pbuf = counter_buf;

	ret = wldev_iovar_getbuf(dev, "counters", "", 0, pbuf, 1536);
	...
	/* XXX In LGE ATTOS platform, lines longer than 1280 bytes do not show up */
...
/* Hugh mod 0728 Start */
	cur_rxdfrmmcast = dtoh32(cnt.rxdfrmmcast);
	cur_rxmfrmmcast = dtoh32(cnt.rxmfrmmcast);
	cur_rxmfrmocast = dtoh32(cnt.rxmfrmocast);
