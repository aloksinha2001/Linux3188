...
static int wl_android_get_power_mode(struct net_device *net, char *command, int total_len)
{
	int pm_local = PM_FAST;
	int bytes_written = 0;
	int error;

	error = wldev_get_power_mode(net, &pm_local);
	if (error)
		return -1;
	if (pm_local == PM_OFF)
			pm_local = 1;
			...
			pm_local = 0;
	bytes_written = snprintf(command, total_len, "powermode = %d", pm_local);
	DHD_INFO(("%s: command result is %s (%d)\n", __FUNCTION__, command, bytes_written));
	return bytes_written;
}
...
int wl_android_priv_cmd(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	else if (strnicmp(command, CMD_GETPOWER, strlen(CMD_GETPOWER)) == 0) {
		bytes_written = wl_android_get_power_mode(net, command, priv_cmd.total_len);
	}
	else if (strnicmp(command, CMD_POWERMODE, strlen(CMD_POWERMODE)) == 0) {
#ifdef WL_CFG80211
		bytes_written = wl_cfg80211_set_power_mode(net, command);
#endif
	}
...
#endif
	else if (strnicmp(command, CMD_MACADDR, strlen(CMD_MACADDR)) == 0) {
...
#if 0 //penguin, temporary remove, the iface name should be determin when insert module
//#ifdef CUSTOMER_HW2
	if (!iface_name[0]) {
